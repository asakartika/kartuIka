...
class Pembayaran extends CI_Controller {
	...
	{	
		...
		if (count($cek)>0){
			...
			$data = array(
			'id_pembayaran' => $ck->id_pembayaran,
			'namaAlumni' => $ck->nama,
			'alamatPengirim' => $ck->alamat,
			'noHp' => $ck->noHp,
			'hargaOngkir' => $ck->hargaOngkir,
			'buktiPembayaran' => $ck->buktiPembayaran,
			'kodePos' => $ck->kodePos,
			'strataPendidikan' => $ck->strataPendidikan,
			'totalPembayaran' => $ck->totalPembayaran,
			'pesan'=> '',
			);	
				$data['hargaKartu']=  $data['totalPembayaran']-$data['hargaOngkir'];
